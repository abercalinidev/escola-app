...
import { salvarAluno } from "@/app/services/aluno/alunoService";
...
export default function CadastroAluno() {
  ...
  const [representantesDisponiveis, setRepresentantesDisponiveis] = useState<any[]>([]);
  ...
  const carregarRepresentantes = async () => {
    try {
      ...
      setRepresentantesDisponiveis(dados);
    } catch (error) {
  ...
  // Filtrar representantes pelo nome
  const representantesFiltrados = representantesDisponiveis.filter((r) =>
    `${r.nome} ${r.sobrenome}`.toLowerCase().includes(filtroRepresentante.toLowerCase())
  );
      ...
      >
        ...
        >
            ...
            {series.map((s) => (
              ...
              >
                <ButtonText
                  style={{
                    fontWeight: "700",
                    color: aluno.serieAluno === s.value ? "#fff" : "#1e293b",
                  }}
                >
                  {s.label}
                </ButtonText>
          ...
          {representantesFiltrados.map((r) => (
            ...
            >
              <Button
                variant={aluno.representantes.some((rep) => rep.id === r.id) ? "solid" : "outline"}
                size="sm"
                action="primary"
                onPress={() =>
              ...
              >
                <ButtonText
                  style={{
                    fontWeight: "700",
                    color: aluno.representantes.some((rep) => rep.id === r.id)
                      ? "#fff"
                      : "#1e293b",
                  }}
                >
